defmodule MP3NameCleaner do
  ...
  def input(args) do
    ...
    if [] == parsed or {:ok, true} == Keyword.fetch(parsed, :help) do
        IO.puts "Specify args as such: --root_path <root_dir> or -simulate <name>"
        System.halt(0)
    end
    ...
    if params == {} do
      params = 
        case Keyword.fetch(parsed, :root) do
          ...
          :error -> raise "Root path not specified"
        end
    end
      ...
      fn {k,v} ->
        ...
        |> Enum.map(
          fn w ->
            ...
            if w in @artist_abbr do
              w
            else
              String.capitalize(w) # Titlecase each word
            end
          end)
        |> Enum.join(" ")
